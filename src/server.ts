...
/* UDP Handler */
export async function receivePacket(data: Buffer, rInfo: RemoteInfo) {
...
async function handlePacket(packetId: number, player: Player, data: any): Promise<void> {
    ...
    try {
        ...
        if (config.debug.logPackets) {
            ...
            logger.logToFile(`[UDP] Accepted ${framework} at ${Date.now()}`, `inbound_packets`, true);
        }
        ...
        if (config.debug.debugLevel > 1 && !server.unhandledPackets[packetId]) {
            logger.warn("Unhandled packet exception:", exception); // Log the exception data to the console.
