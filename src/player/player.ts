...
export default class Player {
    ...
    async login(uid: string): Promise<void> {
        ...
        const rowData: any = global.fetch(`SELECT *
                                           FROM players
                                           WHERE uid = '${uid}';`);
        if (rowData == undefined) {
            const defaultData: Account = defaultPlayerData(uid); // TODO: Implement default inventory data.
            global.query(`INSERT INTO players (uid, data, inventory)
                          VALUES ('${uid}', '${base64Encode(JSON.stringify(defaultData))}', 'e30');`);

            this.info = defaultData.userData;
            this.stats = defaultData.statistics;
            this.gacha = defaultData.gacha;
            this.inventory = defaultData.inventory;
        } else {
            const defaultData: Account = defaultPlayerData(uid);
            const plrDecoded: Account = JSON.parse(base64Decode(rowData.data));
            this.info = plrDecoded.userData || defaultData.userData;
            this.stats = plrDecoded.statistics || defaultData.statistics;
            this.gacha = plrDecoded.gacha || defaultData.gacha;
            this.inventory = plrDecoded.inventory || defaultData.inventory;
        }
    ...
    async logout(): Promise<void> {
        ...
        global.query(`UPDATE players
                      SET data='${base64Encode(JSON.stringify(serialized))}'
